<?php
// admin/attendance/attendance_reports.php
ob_start();
...
<div class="container-fluid px-4 mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="fw-bold mb-0">
            <i class="bi bi-file-earmark-bar-graph me-2 text-primary"></i>Attendance Reports
        </h2>
            ...
            <form method="GET" class="row g-3 align-items-end">
                ...
                <div class="col-sm-6 col-md-2">
                    ...
                    <input type="date" class="form-control form-control-sm"
                           name="date_from" value="<?= htmlspecialchars($date_from) ?>" required>
                </div>

                <div class="col-sm-6 col-md-2">
                    <label class="form-label small mb-1">Date To</label>
                    <input type="date" class="form-control form-control-sm"
                           name="date_to" value="<?= htmlspecialchars($date_to) ?>" required>
                </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="card text-center border-primary">
                <div class="card-body py-3">
                    <div class="fs-2 fw-bold text-primary"><?= count($reportData) ?></div>
        ...
        <div class="col-6 col-md-3">
            <div class="card text-center border-success">
                <div class="card-body py-3">
                    <div class="fs-2 fw-bold text-success"><?= $totalIn ?></div>
        ...
        <div class="col-6 col-md-3">
            <div class="card text-center border-warning">
                <div class="card-body py-3">
                    <div class="fs-2 fw-bold text-warning"><?= $totalOut ?></div>
        ...
        <div class="col-6 col-md-3">
            <div class="card text-center border-info">
                <div class="card-body py-3">
                    <div class="fs-2 fw-bold text-info"><?= $uniqueStudents ?></div>
    ...
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center">
            <span class="fw-semibold">
                Results: <?= date('M d, Y', strtotime($date_from)) ?>
                <?= $date_from !== $date_to ? ' — ' . date('M d, Y', strtotime($date_to)) : '' ?>
            </span>
            <span class="text-muted small"><?= count($reportData) ?> record(s)</span>
        </div>
                ...
                <table class="table table-hover align-middle mb-0" id="report-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="small"><?= htmlspecialchars($row['student_code']) ?></td>
                            <td class="small">
                                <?= htmlspecialchars($row['course']) ?><br>
                                <span class="text-muted"><?= htmlspecialchars($row['year_level'] . ' — ' . $row['section']) ?></span>
                            </td>
                            <td class="small"><?= htmlspecialchars($row['session_name']) ?></td>
                            <td>
                                <span class="badge <?= $row['attendance_type'] === 'IN' ? 'bg-primary' : 'bg-warning text-dark' ?>">
                                    <?= $row['attendance_type'] ?>
                                </span>
                            </td>
                            <td class="small"><?= htmlspecialchars($row['scan_method']) ?></td>
                            <td class="small"><?= date('M d, Y', strtotime($row['scan_time'])) ?></td>
                            <td class="small"><?= date('h:i:s A', strtotime($row['scan_time'])) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
