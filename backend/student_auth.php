...
function send_student_activation_otp(array $student): array
{
    ...
    if (!can_request_student_otp($student['id'])) {
    ...
    $stmt->execute([$student['id'], $otpCode, 'activation', $expiresAt, 'pending']);

    if (!send_student_otp_email($student, $otpCode, OTP_EXPIRY_MINUTES)) {
        return ['success' => false, 'message' => 'Unable to send OTP email. Please contact the administrator.'];
    }

    $_SESSION['student_pending_activation'] = $student['id'];
    return ['success' => true, 'message' => 'OTP was sent to your registered email address.'];
}
...
function student_verify_otp(string $otp): array
{
    ...
    $stmt->execute([$student['id'], 'activation']);
    $record = $stmt->fetch();
    ...
    if ($record['status'] !== 'pending') {
        return ['success' => false, 'message' => 'This OTP has already been used or expired.'];
    }
